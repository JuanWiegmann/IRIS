...
### What IRIS IS
IRIS is an MCP middleware server — a "personality card" that follows the user across ALL LLMs (Claude, Copilot, ChatGPT). It's invisible. It doesn't DO anything for the user. It only KNOWS things about the user and provides that knowledge to whatever LLM the user is talking to.

The user's LLM calls IRIS via MCP to get context ("how should I talk to this person?"), validates its own drafts blindly via `check_draft`, and logs final outputs for future retrieval. IRIS is pure logic + data. No LLM runs inside IRIS.

### How Understanding Evolved This Session
1. Started: "Build a multi-agent system with Ollama + Bedrock Claude"
2. Shifted: "Wait, the user's LLM is powerful enough — IRIS doesn't need its own LLM"
3. Landed: "IRIS is a pure MCP server. Logic + data. The user's LLM does ALL reasoning."
4. Refined: "IRIS is personalization + light memory ONLY. Not a task executor."

### The Self-Check Trick
`check_draft` makes the LLM call IRIS thinking it's an external validator. IRIS compares the draft against stored profile rules deterministically. The LLM gets independent feedback it couldn't give itself — because you can't judge your own output in the same context. No LLM needed inside IRIS for this.

### Onboarding Philosophy (Target-Based GATE)
IRIS defines research-backed TARGETS (what must be known about the user). Each target has a BARRIER (minimum evidence to count as "satisfied"). The LLM sees open targets and FREELY chooses strategy — edge-case questions, binary, direct ask, observation. IRIS never dictates HOW to ask. Only WHAT needs to be known.
...
- Layer 2: Advanced clients (Claude). MCP sampling — IRIS orchestrates with fresh context per step.

### Scope
Personalization + light cross-session memory. NOT task execution. NOT workflow orchestration. IRIS answers "how should I talk to this person" — never "do this task."

## 2. Decisions Made This Session

| Decision | Why | Rejected |
|----------|-----|----------|
| IRIS = MCP server, no internal LLM | User's LLM is powerful enough, zero install | Multi-agent with Ollama |
| User's LLM does ALL reasoning | Best quality from main model | IRIS generates responses |
...
docs/diagrams/iris_system_logic.tex — LaTeX system diagram
docs/diagrams/iris_logic_v6.pdf     — Compiled current version
